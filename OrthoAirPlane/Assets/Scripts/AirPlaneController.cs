...
public class AirPlaneController : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        rigid.useGravity = (Input.GetButton("Accelerate")) ? false : true;
        // When flying reset the angularVelocity so that we won't get twitchy movement.
        rigid.angularVelocity = (Input.GetButton("Accelerate")) ? Vector3.zero : rigid.angularVelocity;
    ...
    {
        if (Input.GetButton("Accelerate"))  flyForce.IncreaseValue(maxFlyForce, flyAcceleration * Time.deltaTime); 
        if (!Input.GetButton("Accelerate")) flyForce.DecreaseValue(flyDeacceleration * Time.deltaTime); 
    }
